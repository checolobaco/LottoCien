...
export default function Dashboard() {
  ...
  // Check if purchase is blocked (1 hour before/after draw)
  const isPurchaseBlocked = useCallback((): boolean => {
    if (!raffleState) return false;
    if (raffleState.winningNumber !== null) return true; // Block if already drawn
    if (!raffleState.drawDate) return false;

    const now = new Date();
    const drawTime = new Date(raffleState.drawDate);
  ...
  const handleContactSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setContactError(err instanceof Error ? err.message : "Error al enviar el mensaje");
    } finally {
  ...
  const handleSelectNumber = (number: string, ticketStatus: string) => {
    ...
    if (isPurchaseBlocked()) return; // Disallow selection if purchase is blocked
  ...
  return (
    <div className="flex-1 flex flex-col min-h-screen bg-slate-950 text-slate-100 relative overflow-hidden">
      ...
      <main className="max-w-7xl mx-auto px-4 py-6 sm:px-6 flex-1 w-full grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <section className="lg:col-span-8 space-y-6">
          ...
          <div className="glass-panel rounded-2xl p-6 shadow-2xl">
            ...
            {raffleState?.winningNumber && (
              <div className="bg-rose-500/10 border border-rose-500/30 rounded-xl p-4 text-rose-400 text-xs flex items-start gap-2.5 mb-6">
                <Info size={16} className="shrink-0 mt-0.5" />
                <div>
                  <span className="font-bold">Sorteo Finalizado:</span> Las compras para este sorteo se encuentran cerradas ya que los ganadores han sido calculados. Por favor espera a que el administrador reinicie la rifa.
                </div>
              </div>
            )}

            {isPurchaseBlocked() && !raffleState?.winningNumber && (
              <div className="bg-amber-500/10 border border-amber-500/30 rounded-xl p-4 text-amber-400 text-xs flex items-start gap-2.5 mb-6 animate-pulse">
                <Clock size={16} className="shrink-0 mt-0.5" />
                <div>
                  <span className="font-bold">Compras deshabilitadas temporalmente:</span> No se permiten compras desde 1 hora antes hasta 1 hora después del sorteo programado ({formattedDrawDate || "hora del sorteo"}).
                </div>
              </div>
            )}
              ...
              <div className="grid grid-cols-10 gap-2 sm:gap-2.5 aspect-square">
                {tickets.map((ticket) => {
                  ...
                  return (
                    <button
                      key={numStr}
                      disabled={isSold || isPending || isPendingApproval || isPurchaseBlocked()}
          ...
          <div className="glass-panel rounded-2xl p-6 shadow-2xl relative overflow-hidden">
            ...
            {selectedNumbers.length > 0 ? (
              <div className="space-y-6">
                {isPurchaseBlocked() && (
                  <div className="bg-amber-500/10 border border-amber-500/30 rounded-xl p-3 flex items-start gap-2 text-amber-400 text-xs">
                    <Clock size={16} className="shrink-0 mt-0.5" />
                    <span>Las compras y pagos están deshabilitados temporalmente por proximidad o finalización del sorteo.</span>
                  </div>
                )}
                ...
                {user ? (
                  <button
                    ...
                    disabled={paymentLoading || (paymentMethod === "TRANSFER" && !receiptFile) || isPurchaseBlocked()}
      ...
      <button
        ...
        className="fixed bottom-6 right-6 z-40 bg-gradient-to-r from-emerald-500 to-teal-500 hover:from-emerald-400 hover:to-teal-400 text-slate-950 p-4 rounded-full shadow-2xl shadow-emerald-500/20 hover:shadow-emerald-500/40 transition-all hover:scale-110 active:scale-95 group flex items-center gap-2 cursor-pointer font-bold text-sm"
      ...
      {showContactModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm">
          <div className="glass-panel w-full max-w-md flex flex-col rounded-2xl shadow-2xl overflow-hidden border border-slate-800 animate-scale-up">
            <div className="p-6 border-b border-slate-800 flex justify-between items-center bg-slate-900/40">
            ...
            <form onSubmit={handleContactSubmit} className="p-6 space-y-4">
              ...
              <div className="space-y-1.5">
                ...
                <input
                  type="email"
                  required
                  disabled={!!user || contactLoading}
                  value={contactEmail}
                  onChange={(e) => setContactEmail(e.target.value)}
                  className="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-2.5 text-sm text-slate-100 focus:outline-none focus:border-emerald-500 transition-colors disabled:opacity-60 disabled:cursor-not-allowed"
              ...
              <div className="space-y-1.5">
                ...
                <textarea
                  ...
                  className="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-2.5 text-sm text-slate-100 focus:outline-none focus:border-emerald-500 transition-colors resize-none"
                  placeholder="Escribe aquí tu consulta o inconveniente en detalle..."
                />
              </div>
              ...
              <button
                ...
                className="w-full mt-2 bg-gradient-to-r from-emerald-500 to-teal-500 hover:from-emerald-400 hover:to-teal-400 disabled:opacity-50 text-slate-950 font-bold py-3 px-4 rounded-xl text-sm transition-all flex items-center justify-center gap-2 cursor-pointer shadow-lg shadow-emerald-500/10 active:scale-98"
              >
                {contactLoading ? (
                  <div className="w-5 h-5 border-2 border-slate-950 border-t-transparent rounded-full animate-spin" />
                ) : (
